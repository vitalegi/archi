...
public class FileSystemWorkspaceLoader {
    ...
    protected Workspace map(it.vitalegi.archi.workspace.loader.model.Workspace ws) {
        ...
        var pairs = new ArrayList<>(ws.getElements().stream().map(this::toPair).collect(Collectors.toList()));
        while (!pairs.isEmpty()) {
            log.info("New cycle");
    ...
    protected boolean apply(Workspace out, ElementPair pair) {
        ...
        if (StringUtil.isNullOrEmpty(parentId)) {
            ...
            throw new IllegalArgumentException("Can't add to top level " + pair.getOut());
        }
